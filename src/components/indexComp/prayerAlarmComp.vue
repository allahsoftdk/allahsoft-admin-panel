...
export default {
    ...
    methods: {
        async createPrayerAlarm() {
            axiosInstance.post("/api/prayer_alarm/", { prayerAlarm: this.alarmName })
                .then((res) => {
                    this.errorMessage = "";
        ...
        deletePrayerAlarm(alarmId: number, alarmName: string) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axiosInstance.delete("/api/prayer_alarm/" + alarmId, {})
                        .then(() => {
        ...
        updatePrayerAlarm(alarmId: number, alarmName: string) {
            Swal.fire({
                ...
                console.log(result);
                if (result.isConfirmed) {
                    axiosInstance.put("/api/prayer_alarm/" + alarmId, { setPrayerAlarm: result.value?.newAlarm })
                        .then(() => {
        ...
        <div class="col-12 pb-3 border-bottom">
            <div class="row">
                ...
                <div class="col-1 d-flex justify-content-end"><button type="submit" class="btn btn-success text-white"
                        @click="createPrayerAlarm()"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                ...
                <tbody>
                    <tr v-for="alarm in prayerAlarm.getPrayerAlarm">
                        ...
                        <td><button type="button" class="btn btn-info text-white"
                                @click="updatePrayerAlarm(alarm.id, alarm.prayerAlarm)"><svg
                        ...
                        <td><button type="button" class="btn btn-danger"
                                @click="deletePrayerAlarm(alarm.id, alarm.prayerAlarm)"><svg
                                    xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
