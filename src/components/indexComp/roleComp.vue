...
export default {
    ...
    methods: {
        async createRole() {
            axiosInstance.post("/api/role/", { role: this.roleName })
                .then((res) => {
                    this.errorMessage = "";
        ...
        deleteRole(roleId: number, roleName: string) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axiosInstance.delete("/api/role/" + roleId, {})
                        .then(() => {
        ...
        updateRole(roleId: number, roleName: string) {
            Swal.fire({
                ...
                console.log(result);
                if (result.isConfirmed) {
                    axiosInstance.put("/api/role/" + roleId, { name: result.value?.newRole })
                        .then(() => {
        ...
        <div class="col-12 pb-3 border-bottom">
            <div class="row">
                ...
                <div class="col-1 d-flex justify-content-end"><button type="submit" class="btn btn-success text-white"
                        @click="createRole()"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                ...
                <tbody>
                    <tr v-for="rolee in role.getRole">
                        ...
                        <td><button type="button" @click="updateRole(rolee.id, rolee.role)"
                                class="btn btn-info text-white"><svg xmlns="http://www.w3.org/2000/svg" width="16"
                        ...
                        <td><button type="button" @click="deleteRole(rolee.id, rolee.role)" class="btn btn-danger"><svg
